...
float arraySumVector(float* values, int N) {
  ...
  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    ...
    _cs149_vadd_float(result, current_float, result, mask_all);
  }

  int n_steps = (int)log2(VECTOR_WIDTH);
  for (int i = 0; i < n_steps; i += 1) {
    _cs149_hadd_float(result, result);
    _cs149_interleave_float(result, result);
  }

  float* float_output = new float[VECTOR_WIDTH];
  _cs149_vstore_float(float_output, result, mask_all);
